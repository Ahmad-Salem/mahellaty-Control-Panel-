<?php
    ...
    {
    	$offer_name=mysqli_real_escape_string($connect, $_POST['edit_offer_name']);
    	...
    	$offer_id=mysqli_real_escape_string($connect, $_POST['offer_no']);
    	...
    	//image
    	$fileName = $_FILES["edit_offer_photo"]["name"]; // The file name
		$fileTmpLoc = $_FILES["edit_offer_photo"]["tmp_name"]; // File in the PHP tmp folder
		$fileType = $_FILES["edit_offer_photo"]["type"]; // The type of file it is
		$fileSize = $_FILES["edit_offer_photo"]["size"]; // File size in bytes
		$fileErrorMsg = $_FILES["edit_offer_photo"]["error"]; // 0 = false | 1 = true
		$ext= end(explode('.',$_FILES["edit_offer_photo"]["name"]));//get uploaded file extention

		if(!empty($offer_name)&&!empty($offer_description)&&!empty($shop_id)&&!empty($user_id)&&!empty($offer_id)&&!empty($fileName)&&!empty($offer_duration_from)&&!empty($offer_duration_to)&&!empty($main_page))
		{

			$uploaded_name=date("hisa").''.md5(rand()).'.'.$ext;
			//query update 
			$query_update_offer1="UPDATE `offers` SET `offer_name`='{$offer_name}',`offer_description`='{$offer_description}',`offer_photo`='{$uploaded_name}',`main_page`='{$main_page}',`from_date`='{$offer_duration_from}',`to_date`='{$offer_duration_to}'  WHERE `shop_id`='{$shop_id}' AND id='{$offer_id}' LIMIT 1";
			...
			{	
				...
				{ 
					...
					header("location: edit_offer.php?sid={$shop_id}&&o_id={$offer_id}&&u_id={$user_id}");
				}
		...
		}else if(!empty($offer_name)&&!empty($offer_description)&&!empty($shop_id))
		{
			//query update 
			$query_update_offer2="UPDATE `offers` SET `offer_name`='{$offer_name}',`offer_description`='{$offer_description}',`main_page`='{$main_page}',`from_date`='{$offer_duration_from}',`to_date`='{$offer_duration_to}'  WHERE `shop_id`='{$shop_id}' AND id='{$offer_id}' LIMIT 1";
			$perform_query_update_offer2=mysqli_query($connect,$query_update_offer2);
			if($perform_query_update_offer2)
			{
				$_SESSION['Success_offer_check']="true";
		    	$message="تمت تعديل العرض بنجاح.";
				$_SESSION['message_offer']=$message;
				@unlink($fileTmpLoc); 
		    	header("location: edit_offer.php?sid={$shop_id}&&o_id={$offer_id}&&u_id={$user_id}");
			}else
		...
		}else
		{
			//error
				$_SESSION['Success_offer_check']="false";
			    $message="حدث خطا اثناء تعديل العرض حاول مرة اخري.";
				$_SESSION['message_offer']=$message;
			    @unlink($fileTmpLoc); // Remove the uploaded file from the PHP temp folder
			    header("location: edit_offer.php?sid={$shop_id}&&o_id={$offer_id}&&u_id={$user_id}");
		}
