<?php
    ...
    if(isset($_POST['login_sub']))
    {
    	...
    	$advertisement_gov=mysqli_real_escape_string($connect,$_POST['government']);
		...
		if(!empty($advertisement_duration_from)&&!empty($advertisement_duration_to)&&!empty($advertisement_name)&&!empty($advertisement_description)&&!empty($advertisement_id)&&!empty($fileName)&&!empty($advertisement_gov)&&!empty($advertisement_city))
		{
			//query update 
			$uploaded_name=date("hisa").''.md5(rand()).'.'.$ext;
			$query_update_advertisement1="UPDATE `advertisement` SET `advertisement_title`='{$advertisement_name}',`advertisement_description`='{$advertisement_description}',`advertisement_photo`='{$uploaded_name}',`government`='{$advertisement_gov}',`city`='{$advertisement_city}',`from_date`='{$advertisement_duration_from}',`to_date`='{$advertisement_duration_to}',`cost`='{$advertisement_cost}' WHERE `id`='{$advertisement_id}' LIMIT 1";
			$path="../images/advertisement/{$advertisement_id}";
		...
		}else if(!empty($advertisement_duration_from)&&!empty($advertisement_duration_to)&&!empty($advertisement_name)&&!empty($advertisement_description)&&!empty($advertisement_id)&&!empty($advertisement_gov)&&!empty($advertisement_city))
		{
			//query update 
			$query_update_advertisement2="UPDATE `advertisement` SET `advertisement_title`='{$advertisement_name}',`advertisement_description`='{$advertisement_description}',`government`='{$advertisement_gov}',`city`='{$advertisement_city}',`from_date`='{$advertisement_duration_from}',`to_date`='{$advertisement_duration_to}',`cost`='{$advertisement_cost}' WHERE `id`='{$advertisement_id}' LIMIT 1";
