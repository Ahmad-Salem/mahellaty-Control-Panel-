<?php
	...
	if($_POST['do_action']=="get_important_shop")
	{
		...
		if(!empty($city)&&!empty($government)&&!empty($limit)&&!empty($page))
		{
			...
			$offers_info=array();
			$offer_info=array();
			if(mysqli_num_rows($perform_query_get_shops)>0)
			{
				/****************************************************/
				...
				$list=unique_multidim_array($offers_info,"offer_id");
				...
				array_push($list_unordered_final,array("status"=>"okay"));
				foreach($list_unordered as $key)
				{
					
					array_push($list_unordered_final,$key);
				}
				/*******************************************************************/
				...
				//there's no data
				$offer_info=array("status"=>"end");
				array_push($list_unordered_final,$offer_info);
			}
		...
		echo json_encode($list_unordered_final,JSON_FORCE_OBJECT);
	}
