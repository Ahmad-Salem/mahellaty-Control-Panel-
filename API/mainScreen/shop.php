<?php
	...
	if($_POST['do_action']=="get_shops")
	{
		...
		if(!empty($city)&&!empty($government))
		{
			...
			$query_get_shops="SELECT `users`.`id` as `u_id`, `users`.`user_name` as `u_user_name`, `users`.`email` as `u_email`, `users`.`photo` as `u_photo`, `users`.`telephone1`, `users`.`telephone2`, `users`.`address` as `u_address`, `shop`.`shop_name`,`shop`.`main_page`, `shop`.`address`, `shop`.`description`, `shop`.`user_id`, `shop`.`photo`, `shop`.`lat`, `shop`.`log`, `shop`.`allowed_products`, `shop`.`allowed_offers`,`shop`.`country`, `shop`.`government`, `shop`.`city`, `shop`.`shop_activity`,`shop`.`id` as `shop_id` FROM `shop` INNER JOIN `users` ON `users`.`id`=`shop`.`user_id` WHERE `shop`.`government`='{$government}' AND `shop`.`city`='{$city}' AND `shop`.`country`='{$country}' AND `shop`.`main_page`='1' AND `approve`='1' ";
			// echo $query_get_shops;
