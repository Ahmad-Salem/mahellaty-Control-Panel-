<?php
	...
	if($_POST['do_action']=="get_product_list")
	{
		...
		{
			$query_product_list="SELECT `shop`.`user_id`,`users`.`user_name`,`users`.`email`,`users`.`photo`,`users`.`address`,`users`.`telephone1`,`users`.`telephone2`,`products`.`id`, `product_name`, `product_price`, `shop_id`, `product_photo`, `product_description` FROM `products` INNER JOIN `shop` ON `products`.`shop_id`=`shop`.`id` INNER JOIN `users` ON `shop`.`user_id`=`users`.`id` WHERE `shop`.`id`='{$shop_id}' LIMIT $start, $limit";
