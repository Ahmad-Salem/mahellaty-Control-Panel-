			...
			if($shop_activity=="all")
				{
					...
					if(mysqli_num_rows($perform_query_shops)>0)
					{
						$shop_rec=array("status"=>"okay");
					...
					{
						//there's no data
						$shop_rec=array("status"=>"end");
						array_push($list_unordered_final,$shop_rec);
				...
				}else if($shop_activity=="clothes")
				{
					//clothes here
					/*****************************************************/
					$query_shops_clothes="SELECT `shop`.`id` as `s_id`, `shop_name`, `country`, `address`, `description`, `user_id`, `photo`, `shop_activity`, `lat`, `log`, `allowed_products`, `allowed_offers`, `government`, `city` FROM `shop` INNER JOIN `shop_activities` ON `shop`.`id`= `shop_activities`.`shop_id` WHERE `country`='{$country}' AND `government`='{$government}' AND `city`='{$city}' AND `approve`='1' AND (`shop_activities`.`activity`='ملابس أطفال'  OR `shop_activities`.`activity`='ملابس رجالي'  OR `shop_activities`.`activity`='ملابس حريمي'  OR `shop_activities`.`activity`='أحذية رجالي' OR `shop_activities`.`activity`='أحذية حريمي' ) LIMIT $start, $limit";

					$perform_query_shops_clothes=mysqli_query($connect,$query_shops_clothes);

					
					if(mysqli_num_rows($perform_query_shops_clothes)>0)
					{
