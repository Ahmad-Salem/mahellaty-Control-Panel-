<?php
	...
	if(isset($_POST['search_main_page']))
	{
		$search=mysqli_real_escape_string($connect,$_POST['search_main_page']);
		if($search=="2")
		{
			$query_shop_details="SELECT `id`, `shop_name`, `country`,`government`,`city`, `allowed_products` , `allowed_offers`,`main_page`, `address`,`shop_activity` FROM `shop`";
			// echo $query_shop_details;
		}else if($search=="0" || $search=="1")
		{
			$query_shop_details="SELECT `id`, `shop_name`, `country`,`government`,`city`, `allowed_products` , `allowed_offers`,`main_page`, `address`,`shop_activity` FROM `shop` WHERE  `main_page`='{$search}' ";
			// echo $query_shop_details;
		}
