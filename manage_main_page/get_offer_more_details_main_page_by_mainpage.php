<?php
	...
	if(isset($_POST['search']))
	{
		...
		{
			$query_offer_details="SELECT `offers`.`id`, `offer_name`, `offer_description`, `offer_photo`,`user_id`,`shop_id`, `offers`.`main_page`, `from_date`, `to_date` FROM `offers` INNER JOIN `shop` ON `offers`.`shop_id`=`shop`.`id`  WHERE  offers.`main_page` ='{$search}' ";

		}else if($search=="2")
		{
			$query_offer_details="SELECT `offers`.`id`, `offer_name`, `offer_description`, `offer_photo`,`user_id`,`shop_id`, `offers`.`main_page`, `from_date`, `to_date` FROM `offers` INNER JOIN `shop` ON `offers`.`shop_id`=`shop`.`id`";
		}
